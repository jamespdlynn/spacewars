    ...
    extend.call(Sprite.prototype, {
        ...
        set: function(key, val, options) {
            ...
            if (this.easeX || this.easeY){
                var combinedDelta = getHypotonuse(this.easeX, this.easeY);
        ...
        updateData : function(deltaSeconds){
            return this.accelerate(deltaSeconds);
        },

        accelerate : function(deltaSeconds){
            var data = this.data;

            if (this.atMaxVelocity){
                data.posX += (data.velocityX * deltaSeconds);
                data.posY += (data.velocityY * deltaSeconds);
                return this;
            }

            var accelerationX = Math.cos(data.angle) * this.acceleration;
            var accelerationY = Math.sin(data.angle) * this.acceleration;

            var newVelocityX = data.velocityX + (accelerationX * deltaSeconds);
            var newVelocityY = data.velocityY + (accelerationY * deltaSeconds);
            var combinedVelocity = Math.round(getHypotonuse(newVelocityX, newVelocityY));

            if (combinedVelocity <= this.maxVelocity){
                ...
                this.accelerate(deltaSeconds/2);
                this.accelerate(deltaSeconds/2);
            }
