define(['binaryjs', 'microjs', 'model/schemas', 'model/zone', 'model/player', 'model/missile', 'model/constants', 'model/game'],
    function (binary, micro, schemas, Zone, Player, Missile, Constants, gameData){
        ...
        function onClientUpdate(data){
            ...
            if (data.isAccelerating && userPlayer.get("fuel") <= 0){
                data.isAccelerating = false;
            }

            //Don't bother sending update for inconsequential changes
            var sendUpdate = data.isFiring || data.isAccelerating || angleDiff(userPlayer.get("angle"),data.angle) >= Math.PI/2;
            if (sendUpdate){
