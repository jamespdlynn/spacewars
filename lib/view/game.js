...
function(createjs, Overlay, Planet, UserShip, EnemyShip, Missile, Explosion, Constants, gameData, manifest){
    ...
    var initialized, autorun, background, stage, updateTimeout, userShip, sprites, exhaustSound;
    ...
    var GameView = {
        ...
        run : function(){
            ...
            exhaustSound = createjs.Sound.play("exhaustSound", {loop:-1,volume:0});
    ...
    function removeSprite(model){
        if (userShip && model.equals(userShip.model)){
            ...
            stage.removeChild(sprites[model.toString()]);
    ...
    function onTick(evt){
        ...
        var volume = exhaustSound.getVolume();
        var isAccelerating = userShip && userShip.isAccelerating;

        if (isAccelerating && volume < 1){
            exhaustSound.setVolume(volume+0.05);
        }
        else if (!isAccelerating && volume > 0){
           exhaustSound.setVolume(volume-0.05);
        }
    ...
    function onCollision(data){
        ...
        if (model1){
            ...
            explosion.addEventListener("animationend", function(){
                ...
                if (GameView.isRunning && !userShip){
    ...
    function onMouseDown(){
        if (userShip) userShip.isAccelerating = true;
    }

    function onMouseUp(){
       if (userShip) userShip.isAccelerating = false;
    }
